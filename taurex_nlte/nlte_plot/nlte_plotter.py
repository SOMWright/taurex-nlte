...
class Plotter(object):
    ...
    def plot_fitted_tp(self):
        ...
        for solution_idx, solution_val in self.solution_iter():
            ...
            if 'nlte_temp_profile' in solution_val['Profiles'].keys():
                label_addition = self._get_nlte_label_addition('nlte_temperature')
                nlte_temp_prof = solution_val['Profiles']['nlte_temp_profile'][:]
                nlte_temp_prof_std = solution_val['Profiles']['nlte_temp_profile_std'][:]

                plt.plot(nlte_temp_prof, pres_prof, color=self.cmap(float(solution_idx) / self.num_solutions),
                         label=label + label_addition)
                plt.fill_betweenx(pres_prof, nlte_temp_prof - nlte_temp_prof_std, nlte_temp_prof + nlte_temp_prof_std,
                                  color=self.cmap(float(solution_idx) / self.num_solutions), alpha=0.5)

            label_addition = self._get_nlte_label_addition('physics_temperature')
            temp_prof = solution_val['Profiles']['temp_profile'][:]
            temp_prof_std = solution_val['Profiles']['temp_profile_std'][:]
            pres_prof = solution_val['Profiles']['pressure_profile'][:] / 1e5
            plt.plot(temp_prof, pres_prof, color=self.cmap(float(solution_idx) / self.num_solutions),
                     label=label + label_addition)
            plt.fill_betweenx(pres_prof, temp_prof - temp_prof_std, temp_prof + temp_prof_std,
                              color=self.cmap(float(solution_idx) / self.num_solutions), alpha=0.5)
    ...
    def plot_forward_tp(self):
        ...
        label = self._get_nlte_label_addition('physics_temperature')[1:]
        temp_prof = solution_val['Profiles']['temp_profile'][:]
        pres_prof = solution_val['Profiles']['pressure_profile'][:] / 1e5
        plt.plot(temp_prof, pres_prof, label=label)

        if 'nlte_temp_profile' in solution_val['Profiles'].keys():
            label = self._get_nlte_label_addition('nlte_temperature')[1:]
            temp_prof = solution_val['Profiles']['nlte_temp_profile'][:]
            plt.plot(temp_prof, pres_prof, label=label)
    ...
    def plot_fitted_spectrum(self, resolution=None):
        ...
        for solution_idx, solution_val in self.solution_iter():
            ...
            plt.scatter(wlgrid, obs_spectrum, marker='d', zorder=1,
                        **{'s': 10, 'edgecolors': 'grey', 'c': self.cmap(float(solution_idx) / N)})

            self._generic_plot(binned_grid, native_grid, solution_val['Spectra'], resolution=resolution,
                               color=self.cmap(float(solution_idx) / N), label=label)
    ...
    def plot_fitted_contrib(self, full=False, resolution=None):
        ...
        for solution_idx, solution_val in self.solution_iter():
            ...
            plt.errorbar(wlgrid, obs_spectrum, error, lw=1, color='black', alpha=0.4, ls='none', zorder=0,
                         label='Observed')
    ...
    def _get_nlte_label_addition(self, key):
        if key in self.fd['ModelParameters']['Temperature'].keys():
            label_addition = " " + self.fd['ModelParameters']['Temperature'][key][
                ()].lower().decode("utf-8") + " temperature"
        else:
            label_addition = ""
...
def main():
    ...
    parser.add_argument("-P", "--plot-posteriors", dest="posterior", default=False, help="Plot fitting posteriors",
                        action='store_true')
    parser.add_argument("-x", "--plot-xprofile", dest="xprofile", default=False, help="Plot molecular profiles",
                        action='store_true')
    parser.add_argument("-D", "--plot-cprofile", dest="cprofile", default=False, help="Plot condensate profiles",
                        action='store_true')
    parser.add_argument("-t", "--plot-tpprofile", dest="tpprofile", default=False, help="Plot Temperature profiles",
                        action='store_true')
    parser.add_argument("-d", "--plot-tau", dest="tau", default=False, help="Plot optical depth contribution",
                        action="store_true")
    parser.add_argument("-s", "--plot-spectrum", dest="spectrum", default=False, help="Plot spectrum",
                        action='store_true')
    parser.add_argument("-c", "--plot-contrib", dest="contrib", default=False, help="Plot contrib", action='store_true')
    parser.add_argument("-C", "--full-contrib", dest="full_contrib", default=False, help="Plot detailed contribs",
                        action="store_true")
